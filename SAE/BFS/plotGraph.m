...
ranks = '';
%ranks = '{rank=same 1}{rank=same 2 3}';
%graphStrDot = 'digraph{node [shape=circle,style=bold] rankdir=LR;';
graphStrDot = 'digraph{node [style=bold]';
bracket = '}';
...
%OCTAVE
system('fdp -Tjpeg output.dot > output.jpeg');
imshow(imread('output.jpeg'));

%MATLAB
%!C:\Graphviz2.38\bin\fdp.exe -Tjpeg output.dot -o output.jpeg
%imshow(imread('output.jpeg'))
