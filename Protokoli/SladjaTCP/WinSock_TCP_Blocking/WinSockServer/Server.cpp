...
int  main(void) 
{
    ...
    do
    {
        ...
        {
            if (FD_ISSET(listenSocket, &readfds) && brojKlijenata < 10)
            {
        ...
        {
            FD_SET(acceptedSocket[i], &readfds2);
        }
        ...
        else 
        {
            for (int i = 0; i < brojKlijenata; i++)
            {
                if (FD_ISSET(acceptedSocket[i], &readfds2))
                {
                    // Receive data until the client shuts down the connection
                    iResult = recv(acceptedSocket[i], recvbuf, DEFAULT_BUFLEN, 0);
                    if (iResult > 0)
                    {
                        COMMAND* cmd = (COMMAND*)recvbuf;   //castovali smo command-u nazad, ovde sa strlicom pristupamo
                        COMMAND cmd2 = *(COMMAND*)recvbuf;  //druga opcija, ovde sa tackom pristupamo
                        ...
                        sprintf_s(recvbuf, "Trenutni rezultat nakon operacije je: %d", rezultat);
        ...
        // clearujemo fd setove nakon svake iteracije
        FD_CLR(listenSocket, &readfds);
        for (int i = 0; i < brojKlijenata; i++)
        {
            FD_CLR(acceptedSocket[i], &readfds2);
        }
    } while (1);
