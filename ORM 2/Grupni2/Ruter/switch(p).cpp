...
{
	unsigned char kopija[1518];
	memset(kopija, 0, 1518);	//setujem celu memoriju na 0
	...
	//checksumu izracunati
	//1. postavimo checksumu na nula
	//2. pozovemo funkciju da izracuna
	//3. upisemo novu checksumu
	...
	unsigned int min_metric = 100000;
	int port = -1;
	...
	for (int i = 0; i < table_counter; i++)
	{
		if (check_address_in_network(ih->dst_addr, routing_table[i].network_address, routing_table[i].subnet_mask))
		{
			if (min_metric > routing_table[i].metric)
			{
				...
				port = routing_table[i].output_port;
			}
		}

	}

	send_packet(port, packet_header, packet_data);
