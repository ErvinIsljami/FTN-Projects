...
//vezbe 7 strana 3
pcap_dumper_t* measurements;
pcap_dumper_t* switches;
...
int main()
{
	...
	//vezbe 7 strana 3
	measurements = pcap_dump_open(device_handle, "measurements.pcap");
	if (measurements == NULL)
	{
		printf("\n Error opening output file\n");
		return -1;
	}
	switches = pcap_dump_open(device_handle, "switches.pcap");
	if (switches == NULL)
	{
		printf("\n Error opening output file\n");
		return -1;
...
void packet_handler(unsigned char* param, const struct pcap_pkthdr* packet_header, const unsigned char* packet_data)
{
	...
	char network_address[] = { 192, 169, 1, 0 };
	char subnet_mask[] = { 255, 255, 255, 0 };
	
	for (int i = 0; i < 4; i++)
	{
		if (network_address[i] & subnet_mask[i] != ih->src_addr[i] & subnet_mask[i])
			return;	//odbacujemo paket
	}
	...
	if (app_data[0] >> 4 == 1) //meas message
	{
		...
		printf("Id merenja: %d, vrednost merenja: %d V\n", poruka->id, poruka->value);

		pcap_dump((unsigned char*)measurements, packet_header, packet_data);
	...
	else					  //switch message
	{
		...
		if (poruka->value == true)
			printf("Swtich id: %d clsoed.\n", poruka->id);
		else
			printf("Switch id: %d opened.\n", poruka->id);

		pcap_dump((unsigned char*)switches, packet_header, packet_data);
