...
int main()
{
	do	//tacka 3.
	{
		...
		char filter[200];
		char buffer[200];
		printf("Unesit rezim, 0 - normalni, 1 - promsc.\n");
		gets_s(buffer, 200);
		rezim = atoi(buffer);
		printf("Unesit vreme u milisekundama.\n");
		gets_s(buffer, 200);
		vreme = atoi(buffer);
		printf("Unesit filter po kome zelite da filtrirate pakete koristeci berklijevu sintaksu.\n");
		gets_s(filter, 200);
		...
		if ((device_handle = pcap_open_live(device->name, // name of the device
			...
			rezim, // promiscuous mode, ako teba normal rezim onda 0
			vreme, // read timeout
