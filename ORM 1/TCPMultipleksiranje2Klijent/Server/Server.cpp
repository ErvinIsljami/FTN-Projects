...
// Check if socket is successfully created
	...
	do
	{
		...
		do
		{
			fd_set readfds;		//set za citanje
			FD_ZERO(&readfds);
			FD_SET(acceptedSocket, &readfds);	//stavljas socket u set za citanje
			FD_SET(acceptedSocket2, &readfds);	//stavljas socket u set za citanje
			...
			{
				if (FD_ISSET(acceptedSocket, &readfds))		//ako je spreman da primi poruku, primi
				{
					...
					if (iResult > 0)	// Check if message is successfully received
					{
						...
						{
							if (dataBuffer[i] == 'a' || dataBuffer[i] == 'e' || dataBuffer[i] == 'i' || dataBuffer[i] == 'o' || dataBuffer[i] == 'u')
							{
								brojSamoglasnika++;
							}
						}

						sprintf_s(dataBuffer, "Broj samoglasnika u poruci koju ste mi poslali je: %d.", brojSamoglasnika);
				...
				if (FD_ISSET(acceptedSocket2, &readfds))		//ako je spreman da primi poruku, primi
				{
					...
					if (iResult > 0)	// Check if message is successfully received
					{
						...
						int brojSamoglasnika2 = 0;
						for (int i = 0; i < strlen(dataBuffer2); i++)
						{
							if (dataBuffer2[i] == 'a' || dataBuffer2[i] == 'e' || dataBuffer2[i] == 'i' || dataBuffer2[i] == 'o' || dataBuffer2[i] == 'u')
							{
								brojSamoglasnika2++;
							}
						}

						sprintf_s(dataBuffer2, "Broj samoglasnika u poruci koju ste mi poslali je: %d.", brojSamoglasnika2);
			...
			FD_CLR(acceptedSocket2, &readfds);

		} while (true);	//OVDE TRUE
